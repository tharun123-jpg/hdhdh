...
public class FloatButton extends SettingsDrawable {
    public FloatSetting setting;
    public float value;
    public float x;
    public boolean dragging;

    public FloatButton(FloatSetting floatSetting, float f, float f2, float f3, float f4) {
        super(floatSetting, f, f2, f3, f4);
    ...
    public void renderButton(int n, int n2, float f, float f2, float f3, float f4, float f5, float f6) {
        ...
        if (dragging) {
            setting.invokeValue(MathUtil.findMiddleValue(setting.getMin() + (setting.getMax() - setting.getMin()) * ((n - getX()) / getWidth()), setting.getMin(), setting.getMax()));
        }
    }
